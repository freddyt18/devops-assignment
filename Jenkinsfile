pipeline {
    ...
    stages {
        ...
        //     steps {
        //         sh """
        //             ansible-playbook playbooks/04_cluster.yml -e "digital_ocean_token=${digital_ocean_token}"
        //         """
        //     }
        // }
    }
    ...
    post {
        success {
            echo """
                curl -X POST "https://api.telegram.org/bot${telegram_token}/sendMessage" -d "chat_id=642027926&text=Infrastructure%20has%20been%20deployed%20successfully."
            """
        }
        failure {
            echo """
                curl -X POST "https://api.telegram.org/bot${telegram_token}/sendMessage" -d "chat_id=642027926&text=Infrastructure%20has%20failed%20to%20deploy."
            """
        }
